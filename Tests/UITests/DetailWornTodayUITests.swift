...
final class DetailWornTodayUITests: XCTestCase {
    func testWornTodayButtonExists() throws {
        ...
        // Wait for collection to render and tap first watch entry
        // Try grid cell first (preferred)
        let gridCell = app.buttons.matching(NSPredicate(format: "identifier BEGINSWITH %@", "GridCellManufacturer-")).firstMatch
        if gridCell.waitForExistence(timeout: 5) {
            gridCell.tap()
        } else {
            // Fallback to list cell
            let listCell = app.cells.firstMatch
            XCTAssertTrue(listCell.waitForExistence(timeout: 5), "Should have at least one watch in collection")
            listCell.tap()
        }
